...
def fetch_and_save_parquet(db, table_name, parquet_path):
    ...
    try:
        ...
        app_logger.info(f"Loaded {table_name} from Supabase.")
        df.to_parquet(parquet_path, index=False)
        app_logger.info(f"{table_name} saved to parquet file at {parquet_path}.")
    except Exception as e:
        app_logger.error(f"Failed to load {table_name} from Supabase: {e}", exc_info=True)
...
def upsert_and_replace_parquet(db, df, table_name, parquet_path):
    ...
    try:
        app_logger.info(f"Upserting local {table_name} data to Supabase.")
        db.upsert_to_supabase(df, table_name)
        df = db.fetch_all_df(table_name)
        df.to_parquet(parquet_path, index=False)
        app_logger.info(f"Replaced local {table_name} parquet file with Supabase data.")
    except Exception as e:
        app_logger.error(f"Failed to upsert or replace {table_name}: {e}", exc_info=True)

def process_data(db, table_name, parquet_path):
    """Process data based on whether the Parquet file exists or not."""
    if not os.path.exists(parquet_path):
        app_logger.info(f"Parquet file for {table_name} not found at {parquet_path}. Fetching and saving new parquet.")
        fetch_and_save_parquet(db, table_name, parquet_path)
    else:
        app_logger.info(f"Parquet file for {table_name} found at {parquet_path}. Reading and upserting.")
...
def db_refresh():
    try:
        ...
        app_logger.info(f"USE_SUPABASE flag is set to {use_supabase}.")
        if not use_supabase:
            app_logger.info("Supabase is disabled. Skipping database operations.")
            return
        ...
        app_logger.error("Error during database refresh process", exc_info=True)