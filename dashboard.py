...
if tab_selection == "Monthly":
    st.title("Stock Portfolio Performance - Monthly")
...
# Handle daily data
    ...
    st.title("Stock Portfolio Performance - Daily")

    # Sort product options for daily data
    daily_product_options = sorted(daily_df['product'].unique().tolist())
    if "Full Portfolio" in daily_product_options:
        daily_product_options.remove("Full portfolio")
        daily_product_options.insert(0, "Full portfolio")

    # Set default index for "Full Portfolio"
    default_index = daily_product_options.index("Full portfolio")
